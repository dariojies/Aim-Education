...
import AdminMascotas from './AdminMascotas.jsx';
...
function BillingRecibos({ showToast }) {
  ...
  async function abrirRectificar(r) {
    ...
    // Solo lo que sigue vivo: una línea rectificada antes no se rectifica dos veces.
    const todas = t.detalle.filter(l => l.cargoId != null);
    const lineas = todas.filter(l => !l.rectificada);
    const yaRectificadas = todas.length - lineas.length;
        ...
        return (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
            ...
            <div style={{ background: 'var(--bg-2)', borderRadius: 20, width: '100%', maxWidth: 620, maxHeight: '90vh', overflowY: 'auto', padding: 24 }}>
              ...
              {R.yaRectificadas > 0 && (
                <p style={{ margin: '0 0 16px', fontSize: 13, fontWeight: 600, color: 'var(--purple)' }}>
                  Este recibo ya tuvo {R.yaRectificadas} línea{R.yaRectificadas !== 1 ? 's' : ''} rectificada{R.yaRectificadas !== 1 ? 's' : ''} antes.
                  Abajo solo aparece lo que aún queda por devolver.
                </p>
              )}
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      const [r, rp] = await Promise.all([
        ...
        // La previsión (lo que se cobrará y aún no está generado) solo tiene
        // sentido para un mes concreto, no para "todos los meses".
        todos ? Promise.resolve(null) : fetch(`/api/admin/billing/cargos/prevision?mes=${mesIso}`, { credentials: 'include', cache: 'no-store' }),
      ]);
  ...
  const sections = [
    ...
    },
    {
      heading: "Actividad", items: [
        ...
        { id: "mascotas", label: "Mascotas", icon: <I.Heart /> },
    ...
    <main style={{ paddingTop: 0 }}>
      <div className="admin-layout">
        ...
        <div className="admin-main">
          ...
          {ver("mascotas") && <AdminMascotas showToast={showToast} />}
